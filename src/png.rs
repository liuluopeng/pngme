...
impl std::convert::TryFrom<&[u8]> for Png {
    fn try_from(value: &[u8]) -> Result<Self> {
        // [header]  [chunk1: length  data crc]  [chunk2] [chunk3]

        // 前8个u8是header.
        let (header, mut raw_chunks) = value.split_at(8);

        // check given header:
        for &b in header {}
        if header != Png::STANDARD_HEADER {
            // return Err("ff".to_string());

            return Err(Box::from("bad header"));
        }

        let mut chunks = vec![];

        // length8  type8  datalength  crc8

        loop {
            let raw_length = [raw_chunks[0], raw_chunks[1], raw_chunks[2], raw_chunks[3]];
            let length = u32::from_be_bytes(raw_length);
            // println!("{:?} length is ", length);

            let (chunk, new_raw_chunks) = raw_chunks.split_at(length as usize + 4 + 4 + 4);
        ...
        // let the_chunk = Chunk::try_from(raw_chunks);
...
impl Png {
    ...
    // bytes: Vec<u8> = [13, 80, 78, 71, 13, 10, 26, 10]
    ...
    pub fn remove_first_chunk(&mut self, s: &str) -> Option<Chunk> {
        for i in 0..self.chunks.len() {
            if self.chunks[i].chunk_type().to_string() == s {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        res.append(&mut Self::STANDARD_HEADER.to_vec());
    ...
    pub fn chunk_by_type(&self, s: &str) -> Option<&Chunk> {
        for c in self.chunks.iter() {
            if c.chunk_type().to_string() == s {
                return Some(c);
            }
        }

        return None;
        self.chunks.iter().find(|x| x.chunk_type().to_string() == s)
    }
    ...
    // 把字节写入png文件
    pub fn gen_png_file(&self, file_path: PathBuf) -> io::Result<()> {
        // 打开文件，如果文件不存在则创建文件
...
mod tests {
    ...
    #[test]
    fn test_gen_png_file() {
        let png = Png::try_from(&PNG_FILE[..]).unwrap();
        let _ = png.gen_png_file("ok.png".to_string().into());
    }
